 ...
 *               freeze + alert stand (honest v0 limit).
 *   floor     — SLO floor breached; auto-suspend = kill switch tier 1 (registry
 *               state → suspended) and page the owner.
 */
...
export function computeLadderLevel(s: LadderSignals): LadderVerdict {
  ...
  const floorByBurn = s.budget.measurable && s.budget.burn_ratio >= s.thresholds.floor_burn_ratio;
  if (floorByCycles)
    reasons.push(`${s.consecutiveProbeCycles} consecutive full-cycle probe failures`);
  if (floorByBurn)
    reasons.push(`burn_ratio ${s.budget.burn_ratio} >= ${s.thresholds.floor_burn_ratio}`);
  if (floorByCycles || floorByBurn) return { level: 'floor', reasons };
  ...
  if (severeByProbes || severeByMean) return { level: 'severe', reasons };
