...
export const taskContractSchema = {
  ...
  "$defs": {
    ...
    "compensation": {
      "type": "object",
      "title": "CompensationReport",
      "description": "Saga unwind summary, present iff a compensation trigger fired on a task that had completed R2+ writes with declared compensators. Compensators run in reverse (LIFO) of the order the writes completed; each entry names the original write and its compensating dispatch. Human-readable lines are additionally carried in `gaps`.",
      "required": [
        "status",
        "trigger",
        "compensated",
