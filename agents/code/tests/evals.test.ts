 ...
 * code-forge server (the same implementation the dev platform runs).
 */
...
function buildAgent(): Agent {
  const agent = Agent.fromManifest(join(ROOT, 'manifest.yaml'), {
    retriever: noRetriever('code-agent'),
  });
  registerCapabilities(agent, { tools: fixtureToolClient() });
  return agent;
}

describe('code-agent eval suites', () => {
