 ...
 * marshalling over InMemoryTransport against the @acp/mock-tools netsec
 * server (the same implementation the dev platform runs) — pure reads over
 * the fixtures, no store.
 */
...
describe('netsec-agent eval suites', () => {
  it('passes the golden suite with clean citation precision and abstention accuracy', async () => {
    ...
    expect(report.citationPrecision).toBe(1.0);
