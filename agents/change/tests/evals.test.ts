 ...
 * marshalling over InMemoryTransport against the @acp/mock-tools itsm server
 * (the same implementation the dev platform runs), one shared stateful store.
 */
...
function buildAgent(): Agent {
  const agent = Agent.fromManifest(join(ROOT, 'manifest.yaml'), {
    retriever: noRetriever('change-agent'),
  });
  registerCapabilities(agent, { tools: fixtureToolClient() });
  return agent;
}

describe('change-agent eval suites', () => {
