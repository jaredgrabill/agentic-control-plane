/** Tool bindings: the itsm MCP server, addressed by name. */
...
export const ITSM = 'itsm';

/**
 * Item 5: tool calls traverse the Tool Gateway PEP. Item 0c: when the agent's
 * own client secret is configured, each call exchanges the step's delegated
 * token for an `acp:tools` token (the gateway's only accepted audience after
 * the flip). Without a secret (unit tests) the delegated token is sent verbatim.
 */
export function createToolClient(): ToolClient {
  ...
  const tokenProvider =
    clientSecret !== undefined
      ? toolTokenProvider({
          ...
          clientId: process.env.ACP_AGENT_CLIENT_ID ?? 'agent-change-agent',
          clientSecret,
        })
      : undefined;
  return new McpToolClient({
    servers: {
      [ITSM]: {
        url: process.env.ACP_TOOL_SERVER_ITSM_URL ?? 'http://localhost:7106/mcp/itsm',
      },
    },
 ...
 * missing step id is a bug in the caller, not something to paper over with a
 * random key (which would defeat de-duplication).
 */
export function idempotencyKey(ctx: Pick<CapabilityContext, 'stepId'>): string {
